...
SELECT public.td_flag_export('td_stp_entry') ; 
...
from public.td_stp_entry  
where 
(
ssid <=  (SELECT coalesce(max(ssid), date '1970-01-01') FROM  public.td_ss_info WHERE report = 'td_stp_entry' and ( reported = 'P' )) 
and ssid > (SELECT coalesce(max(ssid), date '1970-01-01') FROM  public.td_ss_info WHERE report  = 'td_stp_entry' and ( reported  = 'Y' ))
AND  offline_replace_recovery = '0' --'N'	
) 
OR 
(
ssid <=  (SELECT coalesce(max(ssid), date '1970-01-01') FROM  public.td_ss_info WHERE report = 'td_stp_entry_offline_replace_recovery' and ( reported = 'P' )) 
and ssid > (SELECT coalesce(max(ssid), date '1970-01-01') FROM  public.td_ss_info WHERE report  = 'td_stp_entry_offline_replace_recovery' and ( reported  = 'Y' ))
...
COMMENT ON FUNCTION public.td_stp_entry_export() IS 'Version: 201808231600
