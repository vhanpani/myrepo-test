...
from public.td_svc_exit  
where 
(
ssid <=  (SELECT coalesce(max(ssid), date '1970-01-01') FROM  public.td_ss_info WHERE report = 'td_svc_exit' and ( reported = 'P' )) 
and ssid > (SELECT coalesce(max(ssid), date '1970-01-01') FROM  public.td_ss_info WHERE report  = 'td_svc_exit' and ( reported  = 'Y' ))
	AND  offline_replace_recovery = '0' --'N'	
) OR
(
	ssid <=  (SELECT coalesce(max(ssid), date '1970-01-01') FROM  public.td_ss_info WHERE report = 'td_svc_exit_offline_replace_recovery' and ( reported = 'P' )) 
and ssid > (SELECT coalesce(max(ssid), date '1970-01-01') FROM  public.td_ss_info WHERE report  = 'td_svc_exit_offline_replace_recovery' and ( reported  = 'Y' ))
...
COMMENT ON FUNCTION public.td_svc_exit_export() IS 'Version: 201808231600
